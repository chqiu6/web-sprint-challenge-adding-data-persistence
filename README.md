...
The difference between relational databases and SQL is that relation databases stores our data while SQL access and manipulates our data through queries.

2. Why do tables need a Primary Key?
Tables need a primary key because it allows table to be linked with another table using foreign keys.

3. What is the name given to a table column that references the Primary Key on another table?
A foreign key references the primary key on another table.

4. What do we need in order to have a _many to many_ relationship between two tables?
We need a junction table that acts as a bridge between two other tables by referencing each table's primary keys.
